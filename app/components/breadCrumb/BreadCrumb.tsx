...
const Breadcrumb: React.FC<BreadcrumbProps> = ({ title, category }) => {
  ...
  return (
    <nav className="flex items-center space-x-2 text-sm">
      <Link href="/">
        <p className="text-black font-bold hover:bg-white hover:text-black hover:rounded-lg hover:opacity-75 capitalize p-2">Home</p>
      </Link>
      {isBlogPage && category && (
        <>
          <span className="mx-2">{'>'}</span>
          <Link href={`/category/${encodeURIComponent(category.toLowerCase().replace(/ /g, '-'))}`}>
            <p className="text-black font-bold hover:bg-white hover:text-black hover:rounded-lg hover:opacity-75 capitalize p-1">
          ...
          {title && (
            <>
              <span className="mx-2">{'>'}</span>
              <span className="text-black font-bold capitalize">{decodeURIComponent(title)}</span>
            </>
          )}
        </>
      )}
      ...
      {!isBlogPage && !isCategoryPage && pathSegments.map((segment, index) => {
        ...
        return (
          <React.Fragment key={index}>
            <span className="mx-2">{'>'}</span>
            {isLast ? (
              <span className="text-black capitalize">{decodedSegment}</span>
            ) : (
              <Link href={href}>
                <p className="text-black hover:text-black font-bold hover:bg-white hover:rounded-lg hover:opacity-75 capitalize p-1">
                  {decodedSegment}
                </p>
              </Link>
            )}
